...
// wypisywanie pojedynczego ruchu
void printMove(const Move& m) {
    std::cout << "Ruch: ";
    std::cout << square_to_string(m.from) << " -> " << square_to_string(m.to);

    if (m.captured != NO_PIECE)
        std::cout << " (zbito: " << int(m.captured) << ")";

    if (m.flags & FLAG_PROMOTION)
        std::cout << " (promocja: " << int(m.promotion) << ")";

    if (m.flags & FLAG_CASTLE_KINGSIDE)
        std::cout << " (roszada krótka)";
    if (m.flags & FLAG_CASTLE_QUEENSIDE)
        std::cout << " (roszada długa)";
...
// wypisanie wszystkich ruchów w tablicy lub wektorze
template<typename Container>
void printMoves(const Container& moves) {
    for (const auto& m : moves) {
        printMove(m);
        std::cout << "========================\n";
    }
}
void printMovesArray(Move moves[], int count) {
    for (int i = 0; i < count; i++) {
        const Move& m = moves[i];

        // konwersja pola na notację a1-h8
        auto square_to_string = [](int sq) {
            char file = 'a' + (sq % 8);
            char rank = '1' + (sq / 8);
            return std::string() + file + rank;
            };
