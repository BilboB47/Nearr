...
Move Search::get_best_move(Position& pos, int max_depth) {
    ...
    for (int current_depth = 1; current_depth <= max_depth; current_depth++) {
        ...
        global_best_move = best_move_this_iteration; //zawsze zmienia bo g³êbiej=lepiej
    }
...
int Search::alpha_beta(Position& pos, int depth, int alpha, int beta, int root_depth, int ply) {
    ...
    if (is_repetition(pos.zobristKey)) { //czy jest to powielenie pozycji daje 0 czyli remis (zasada powtózenia pozycji)
        return 0; 
    }
    ...
    TTEntry& tt_value = TT_table[index]; //zawrtoœæ TT takiej pozycji
    Move tt_move = Move();

    if (tt_value.key == pos.zobristKey) { //jeœli to jest faktycznie taka pozycja (index mo¿e zawieraæ kolizje)
        ...
        if (tt_value.depth >= depth) {
            ...
            if (tt_value.flag == TTFlag::Alpha && score <= alpha) return alpha; // Górna granica (fail-low): wynik nie poprawi alpha
            if (tt_value.flag == TTFlag::Beta && score >= beta) return beta;    // Dolna granica (fail-high): przeciwnik ma lepsz¹ opcjê
        }
    }

    if (depth <= 0) return quiescence(pos, alpha, beta); //licz dalej tylko gloœne ruchy

    //-------liczenie wszytkich ruchów--------------------------------------------------
    Move moves[256];
    int count = generateMoves(pos, moves);
    int scores[256];
...
int Search::score_move(Position& pos, Move move, int ply) {
    ...
    if (move.flags & 4) {//jest to bicie 
        ...
        //Bicia maj¹ du¿e value + w zale¿noœci co bije co
    ...
    //jak nie jest to bicie to daje zwykle pst (by po czymœ sortowalo)
